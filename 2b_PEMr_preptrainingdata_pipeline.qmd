---
title: "PEMsamplr_preparing reference data"
format: html
cache: TRUE
---
...
input_path <- PEMprepr::read_fid()$dir_20104020_transect$path_abs
input_path <- PEMprepr::read_fid()$dir_20105020_clean_field_data$path_abs
# location where the final simplified transect layer will be stored
out_dir = PEMprepr::read_fid()$dir_201040_plan_final$path_abs
...
key <- generate_mapkey(
  # data_pts  = fs::path(PEMprepr::read_fid()$dir_20105020_clean_field_data$path_rel, "s1_points_raw.gpkg"),
    data_pts  = fs::path(PEMprepr::read_fid()$dir_20105020_clean_field_data$path_rel, "allpoints.gpkg"),
  write_output = TRUE,
...
# 1. convert points to lines - no neighbours
  out_dir = fs::path(PEMprepr::read_fid()$dir_20105020_clean_field_data$path_rel)
  out_name = "proc_s1_transects5.gpkg"
  processed_transects <- sf::st_read(file.path(out_dir,out_name))
...
####ERRORs here may because the template is not based on the original dem file so raster dimension of all layers are unequal. remove dem.tiff and template.tiff from here and it will run

```{r}
#| eval: false
att_pts <- attribute_points(
  data_pts = processed_pts,
  cov_dir = fs::path(PEMprepr::read_fid()$dir_1020_covariates$path_abs, "5m"),
  write_output = TRUE,
  out_dir = fs::path(PEMprepr::read_fid()$dir_20105030_attributed_field_data$path_abs),
